...
public class ProjectAuditBController extends GenericService{
     ...
     * @return
     */
    @GetMapping("/v{appid}/get/project/send/list")
    public CommonDto<List<Map<String,Object>>> getProjectSendList(String shortName, Integer pageNum, Integer pageSize, @PathVariable Integer appid){
        CommonDto<List<Map<String,Object>>> result  = new CommonDto<>();

        try {
            result = projectAuditBService.getProjectSendList(shortName, pageNum, pageSize, appid);
        }catch (Exception e){
